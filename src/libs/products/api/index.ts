...
export async function getStoreProducts(storeId: number) {
  return isDev ? generateMockProductPreviews() : httpClient.get<ProductPreview[]>(`/stores/${storeId}/products`);
}
...
export async function createProduct(storeId: number, data: CreateProductDto) {
  return isDev ? generateMockProductDetail() : httpClient.post<ProductDetail>(`/stores/${storeId}/products`, data);
}

export async function updateProduct(storeId: number, productId: number, data: UpdateProductDto) {
  return isDev ? generateMockProductDetail() : httpClient.patch<ProductDetail>(`/stores/${storeId}/products/${productId}`, data);
}
